...
- `cut` (optionnel) : la **découpe de mise en place** quand la recette la précise (dans la ligne d'ingrédient ou les étapes : « oignon émincé », « carottes en brunoise », « ail haché », « pommes de terre en rondelles »). Objet `{ "forme": <valeur ci-dessous>, "calibre"?: "fin" | "moyen" | "gros" }`. **Omets `cut`** s'il n'y a pas de découpe au couteau (farine, œufs, liquides, épices en poudre…) ou si elle n'est pas indiquée : n'invente JAMAIS une découpe.
  ...
  **`emince` vs `rondelle` (piège fréquent, conséquence importante sur la recette) :** ces deux formes ne sont PAS interchangeables.
    - `emince` = **tranches fines et plates**, quelle que soit leur orientation : « émincé », « en lamelles », « en fines lamelles », « en tranches fines », « finement tranché / trancher finement », « à la mandoline », toute épaisseur explicitement fine (« ~2 mm », « ~4 mm », « fines tranches »). C'est le cas par DÉFAUT dès qu'on parle de trancher finement (gratin, moussaka, aubergine/courgette en lamelles, pommes de terre pour un gratin…).
    - `rondelle` = **uniquement** des tranches circulaires épaisses obtenues en coupant un légume cylindrique EN TRAVERS, et **seulement** quand la source dit explicitement « rondelles » / « en rondelles » / « rouelles » (carotte, courgette, concombre, banane en rondelles…).
    - En cas de doute, ou si la source dit « tranches / lamelles / trancher / mandoline » SANS écrire « rondelles » → choisis `emince`, jamais `rondelle`. Ne classe en `rondelle` que sur le mot « rondelle(s) » (ou « rouelle(s) ») explicite.
