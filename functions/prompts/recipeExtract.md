...
- `cut` (optionnel) : la **découpe de mise en place** quand la recette la précise (dans la ligne d'ingrédient ou les étapes : « oignon émincé », « carottes en brunoise », « ail haché », « pommes de terre en rondelles »). Objet `{ "forme": <valeur ci-dessous>, "calibre"?: "fin" | "moyen" | "gros" }`. **Omets `cut`** s'il n'y a pas de découpe au couteau (matières grasses comme le beurre, farine, sucre, œufs, liquides, épices en poudre…) ou si elle n'est pas indiquée : n'invente JAMAIS une découpe (on n'émince pas du beurre qu'on fait fondre).
  `forme` : EXACTEMENT une valeur de cette liste FERMÉE (minuscules, sans accent) : `emince` · `cisele` · `hache` · `chiffonade` · `rape` · `des` · `brunoise` · `mirepoix` · `paysanne` · `julienne` · `batonnet` · `lamelle` · `rondelle` · `troncon` · `quartier`.
  La découpe va dans `cut`, **jamais dans `name`** (cf. règle `name`) : « 2 oignons émincés » → name `oignon`, amount `2`, `cut` `{ "forme": "emince" }`.
  **`emince` vs `lamelle` vs `rondelle` (piège fréquent, conséquence importante sur la recette) :** ces trois formes de « tranchage » ne sont PAS interchangeables.
    - `lamelle` = **tranches plates**, coupées dans la longueur ou la largeur d'un légume charnu qu'on tranche « à plat » : aubergine, courgette, pomme de terre pour un gratin ou une moussaka, fenouil… Marqueurs : « en lamelles », « en tranches », « en fines tranches », « trancher / trancher finement », « à la mandoline », épaisseur donnée (« ~3 mm », « ~5 mm »). C'est le cas par DÉFAUT dès qu'on tranche à plat un légume charnu (aubergine/courgette pour gratin ou moussaka, pommes de terre pour gratin).
    - `emince` = **couper en tranches fines** un légume qu'on émince, typiquement oignon, échalote, champignon, poireau, chou, fenouil en fines lanières : réserve `emince` au geste « émincer » nommé explicitement.
    - `rondelle` = **uniquement** des tranches circulaires obtenues en coupant un légume cylindrique EN TRAVERS, et **seulement** si la source dit explicitement « rondelles » / « en rondelles » / « rouelles » (carotte, concombre, banane, courgette en rondelles…).
    - Règle de tri : « émincer » nommé → `emince` ; « rondelle(s) » / « rouelle(s) » nommé → `rondelle` ; tout autre tranchage (tranches, lamelles, trancher, mandoline, épaisseur en mm) → `lamelle`. Ne classe JAMAIS en `rondelle` sans le mot « rondelle(s) » (ou « rouelle(s) »).
