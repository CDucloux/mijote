...
- **COHÉRENCE OBLIGATOIRE.** Toute sous-préparation que tu référence (ligne `{ "component": "X" }` ou mention dans une étape) DOIT exister dans `components` avec SON `yield`, SES ingrédients bruts ET SES étapes de préparation. Ne référence JAMAIS un composant que tu n'as pas créé.
