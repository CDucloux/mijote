...
- `ingredients` / `utensils` : les noms (repris EXACTEMENT de tes listes ci-dessus) utilisés dans l'étape ; `[]` sinon. **Relie CHAQUE ingrédient nommé dans le texte de l'étape**, y compris quand il est désigné par un article ou un pronom (« peler et émincer **l'oignon** », « écraser **l'ail** » → relie `oignon` et `ail`) : un ingrédient présent dans la liste et cité dans le geste ne doit JAMAIS être laissé sans lien. Deux garde-fous seulement : ne relie pas un ingrédient d'une AUTRE phase juste parce qu'il porte le même nom, et respecte le cloisonnement des sections ci-dessous.
