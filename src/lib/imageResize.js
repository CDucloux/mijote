// ─── REDIMENSIONNEMENT DES PHOTOS AVANT IMPORT IA ────────────────────────────
// Levier principal sur la latence : une photo de smartphone brute pèse 4-7 Mo ;
...
export function resizeDimensions(w, h, maxEdge = MAX_EDGE) {
  const scale = Math.min(1, maxEdge / Math.max(w, h || 1));
  return { width: Math.round(w * scale), height: Math.round(h * scale), scale };
}

// File image → { blob (image/jpeg), mediaType }. Décode en respectant l'EXIF (une
// photo portrait ne doit pas arriver couchée : ça dégrade l'OCR), redimensionne
// sur canvas, ré-encode en JPEG. Repli sur le fichier d'origine si l'API canvas
// manque — mais seulement pour un type déjà supporté par l'API vision.
