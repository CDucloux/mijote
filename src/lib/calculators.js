...
// 1. Moule / plat : facteur d'adaptation des quantités entre deux moules
//    (ratio de surface, ou de volume si les deux hauteurs sont fournies).
// 2. Conversions d'unités : masse ↔ volume via la densité de l'ingrédient.

import { normalizeStr } from "./parseIngredient.js";

// ── MOULE ─────────────────────────────────────────────────────────────────────
// Surface (cm²) d'un moule. `d` : { diameter } (rond) ou { length, width } (rect).
export function panArea(shape, d = {}) {
  if (shape === "round") { const r = (Number(d.diameter) || 0) / 2; return Math.PI * r * r; }
  if (shape === "rect") { return (Number(d.length) || 0) * (Number(d.width) || 0); }
  return 0;
}

// Facteur multiplicateur pour passer du moule `orig` au moule `target`.
// Si les DEUX hauteurs sont renseignées (> 0) → ratio de volume, sinon de surface.
// Renvoie null si un moule est incomplet.
export function panFactor(orig, target) {
...
export const MASS_G = { "mg": 0.001, "g": 1, "kg": 1000 };
...
// Devine la densité (g/ml) d'un ingrédient d'après son nom (ou null si inconnu).
...
const unitKind = (u) => (u in MASS_G ? "mass" : u in VOLUME_ML ? "volume" : null);

// Convertit `value` de l'unité `from` vers `to`. `density` (g/ml) requis seulement
// pour une conversion masse ↔ volume. Renvoie null si conversion impossible.
export function convertQuantity(value, from, to, density) {
